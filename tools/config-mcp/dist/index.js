...
async function businessWorkbooks() {
...
async function findWorkbookBySheet(sheetName) {
...
async function backupWorkbook(fileName) {
...
async function upsertRow(sheetName, keyColumn, keyValue, values, options) {
    const workbookName = await findWorkbookBySheet(sheetName);
    const workbook = await loadWorkbook(workbookName);
    const sheet = workbook.getWorksheet(sheetName);
    if (!sheet)
    ...
    if (!keyIndex)
        throw new Error(`Key column ${keyColumn} missing in ${sheetName}`);
    for (const field of Object.keys(values)) {
        if (options?.locked?.has(field))
            throw new Error(`Field is locked: ${field}`);
        if (options?.allowed && !options.allowed.has(field)) {
            throw new Error(`Field is not editable through this tool: ${field}`);
        }
        if (!headers.has(field))
            throw new Error(`Unknown ${sheetName} field: ${field}`);
    }
    let targetRow;
    for (let rowNumber = 4; rowNumber <= sheet.rowCount; rowNumber += 1) {
        const row = sheet.getRow(rowNumber);
        if (text(row.getCell(1).value).trim().startsWith("##"))
            continue;
    ...
    for (const [field, value] of Object.entries(values)) {
        targetRow.getCell(headers.get(field)).value = normalizeCell(value);
    }
    targetRow.commit();
...
export async function batchUpdateRoleStats(updates) {
    if (updates.length === 0)
        throw new Error("No role stat updates supplied");
    const workbookName = await findWorkbookBySheet("RoleList");
    const workbook = await loadWorkbook(workbookName);
    const sheet = workbook.getWorksheet("RoleList");
    if (!sheet)
        throw new Error("RoleList sheet missing");
    const headers = headerMap(sheet);
    const idColumn = headers.get("id");
    if (!idColumn)
        throw new Error("RoleList id column missing");
    const rows = new Map();
    for (let rowNumber = 4; rowNumber <= sheet.rowCount; rowNumber += 1) {
        const row = sheet.getRow(rowNumber);
    ...
    for (const update of updates) {
        if (!update.id || seen.has(update.id))
            throw new Error(`Duplicate or empty role id: ${update.id}`);
        seen.add(update.id);
        const row = rows.get(update.id);
        if (!row)
            throw new Error(`Role not found: ${update.id}`);
        ...
        if (update.faction_tags !== undefined)
            values.faction_tags = update.faction_tags.trim();
        if (update.skill_ids !== undefined)
            values.skill_ids = update.skill_ids.trim();
        if (update.avatar_loc !== undefined)
            values.avatar_loc = update.avatar_loc.trim();
        let rowChanged = false;
        for (const [field, value] of Object.entries(values)) {
            const column = headers.get(field);
            if (!column)
                throw new Error(`RoleList ${field} column missing`);
            if (Number(row.getCell(column).value) !== value)
                rowChanged = true;
...
async function characterContext(roleId) {
    ...
    const ownedSkills = skills.rows.filter((row) => skillIds.has(text(row.values.id)) || text(row.values.owner_id) === roleId);
    const effectIds = new Set(ownedSkills.flatMap((row) => splitRefs(row.values.effect_ids)));
    ...
    const matchingFusions = fusions.rows.filter((row) => splitRefs(row.values.required_tags).some((tag) => roleTags.has(tag)));
    ...
    const educationProgram = educationPrograms.rows.find((row) => text(row.values.level) === educationLevel);
    const lifeRouteGrowth = lifeRoutes.rows.find((row) => text(row.values.route) === lifeRoute);
    const careerGrowth = careers.rows.find((row) => text(row.values.sector) === careerSector);
    return {
        role: role.values,
        ...
        design_rules: {
            ...
            composition: "Character traits -> faction/common tags -> trigger -> skill effects -> buffs -> fusion rules",
            late_graduation: "Master/Phd job skills unlock late and must be stronger after unlock",
        },
    };
}
async function enumNames(enumName) {
    ...
    for (const sheet of workbook.worksheets) {
        ...
        for (let row = 4; row <= sheet.rowCount; row += 1) {
            ...
            if (declaredEnum)
                currentEnum = declaredEnum;
            if (currentEnum !== enumName)
                continue;
    ...
    const block = xml.match(new RegExp(`<enum\\s+name="${enumName}"[\\s\\S]*?<\\/enum>`, "i"))?.[0];
    if (!block)
        return new Set();
...
export async function validateConfig() {
    ...
    function duplicates(rows, key, label) {
        const seen = new Set();
...
export function runLuban() {
    ...
    if (!existsSync(lubanDll))
        throw new Error(`Luban.dll missing: ${lubanDll}`);
...
server.registerTool("list_config", {
    description: "List JojoP Luban workbooks and sheets. Use before reading or changing character, skill, equipment, encounter, event, chapter-run, or growth config.",
    inputSchema: {},
}, async () => {
    const books = [];
    for (const fileName of await businessWorkbooks()) {
...
server.registerTool("read_sheet", {
    description: "Read one Luban sheet as field-keyed rows, excluding ## comment rows. Use for AI analysis before proposing or applying config changes.",
    inputSchema: {
...
server.registerTool("get_character_context", {
    description: "Get one real person's locked source description plus education, life-route, career growth, skills, effects, and related tag fusions. Always use before designing skills from personality. Never alter name or desc.",
    inputSchema: {
        roleId: z.string().min(1),
    },
}, async ({ roleId }) => result(await characterContext(roleId)));
server.registerTool("batch_update_role_stats", {
    description: "Safely batch-write Unity-balanced base stats, faction tags, graduation skill selection, and avatar resource name back to RoleList. Names and descriptions are untouched.",
    inputSchema: {
...
server.registerTool("update_character_design", {
    description: "Safely update AI-designed character mechanics. name/desc/id are locked. Accepts only camp, education_level, life_route, career_sector, faction tags, stats, resource keys, skill_ids, recruitable, sort. Use tags and shared mechanisms instead of hardcoding names.",
    inputSchema: {
        roleId: z.string().min(1),
    ...
    const update = await upsertRow("RoleList", "id", roleId, values, { locked: LOCKED_ROLE_FIELDS, allowed: ROLE_EDITABLE_FIELDS });
    return result({
        ...update,
...
server.registerTool("upsert_run_content", {
    description: "Create or update AI-designed run content in a schema-safe allowlist: chapter rules, rewards, equipment, encounters, Longnan events, and sourced timeline milestones.",
    inputSchema: {
    ...
    const update = await upsertRow(sheet, keyColumn, key, values, { allowed: RUN_CONTENT_FIELDS[sheet] });
    return result({
        ...update,
        rationale,
        next: "Run validate_config, then run_luban.",
...
server.registerTool("promote_skill_draft", {
    description: "Turn one free-form SkillIndex draft row into a runnable skill while preserving its original ## note. Use when the user writes an owner/name/idea directly into an otherwise incomplete Excel row.",
    inputSchema: {
    ...
    for (const effect of effects) {
        ...
        await upsertRow("SkillEffect", "id", text(id), values);
    }
    const workbookName = await findWorkbookBySheet("SkillIndex");
    const workbook = await loadWorkbook(workbookName);
    const sheet = workbook.getWorksheet("SkillIndex");
    if (!sheet)
        throw new Error("SkillIndex sheet missing");
    if (excelRow > sheet.rowCount)
        throw new Error(`SkillIndex row ${excelRow} does not exist`);
    const headers = headerMap(sheet);
    ...
    if (existingId)
        throw new Error(`SkillIndex row ${excelRow} is already a skill: ${existingId}`);
    ...
    for (const [field, value] of Object.entries(skill)) {
        const column = headers.get(field);
        if (!column)
            throw new Error(`Unknown SkillIndex field: ${field}`);
        row.getCell(column).value = normalizeCell(value);
    }
    row.commit();
    const backup = await backupWorkbook(workbookName);
    await workbook.xlsx.writeFile(join(DATA_ROOT, workbookName));
...
server.registerTool("retarget_character_encounter", {
    description: "Retarget one CharacterEncounter row after a user intentionally renamed a RoleList id. This changes only the encounter role_id and rejects duplicates.",
    inputSchema: {
    ...
    if (!sheet)
        throw new Error("CharacterEncounter sheet missing");
    const keyColumn = headerMap(sheet).get("role_id");
    if (!keyColumn)
        throw new Error("CharacterEncounter role_id missing");
    let source;
    for (let rowNumber = 4; rowNumber <= sheet.rowCount; rowNumber += 1) {
        const value = text(sheet.getRow(rowNumber).getCell(keyColumn).value).trim();
        if (value === toRoleId)
            throw new Error(`CharacterEncounter already contains ${toRoleId}`);
        if (value === fromRoleId)
            source = sheet.getRow(rowNumber);
    }
    if (!source)
        throw new Error(`CharacterEncounter does not contain ${fromRoleId}`);
...
server.registerTool("upsert_skill_bundle", {
    description: "Create or update one string-keyed skill and its effects. AI should map personality -> trigger/show_tags -> generic effects/buffs; use {owner}_{action} and {skill}_{effect} ids. Existing unspecified cells are preserved.",
    inputSchema: {
        skill: z.object({
            id: z.string().min(1),
            owner_id: z.string().min(1),
    ...
    for (const effect of effects) {
        ...
        if (!id)
            throw new Error("Every effect requires id");
        if (!effectIds.has(id))
            throw new Error(`Effect ${id} is not referenced by skill.effect_ids`);
    ...
    for (const effect of effects) {
        const { id, ...values } = effect;
        effectUpdates.push(await upsertRow("SkillEffect", "id", text(id), values));
    }
    const { id, ...skillValues } = skill;
    const skillUpdate = await upsertRow("SkillIndex", "id", id, skillValues);
    return result({
        skill: skillUpdate,
...
server.registerTool("upsert_fusion", {
    description: "Create/update a tag-driven fusion/bond. Never hardcode character names as the trigger; use required_tags and grant_skill_id.",
    inputSchema: {
        id: z.string().min(1),
        name: z.string().min(1),
        desc: z.string().min(1),
    ...
    const update = await upsertRow("FusionSkill", "id", id, values);
    return result({ ...update, rationale, next: "Run validate_config, then run_luban." });
});
server.registerTool("validate_config", {
    description: "Validate IDs and references across characters, skills, education, equipment, encounters, run rewards, Longnan events, and fusion skills without changing files.",
    inputSchema: {},
}, async () => result(await validateConfig()));
server.registerTool("run_luban", {
    description: "Run JojoP Luban export after validation. Generates C# and JSON and treats logged |ERROR| as failure.",
    inputSchema: {
    ...
    if (validateFirst) {
        ...
        if (!validation.ok)
            return result({ validation, export: null }, true);
    }
