...
import { batchUpdateRoleStats, runLuban, validateConfig, type RoleStatUpdate } from "./index.js";

type Payload = {
  updates: RoleStatUpdate[];
  exportLuban?: boolean;
};
...
async function main() {
  ...
  if (!payloadPath) throw new Error("Usage: unityBalanceBridge <payload.json> [--export]");
