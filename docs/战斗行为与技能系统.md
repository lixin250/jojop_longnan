...
**表现层定案（纯 AI 可落地）：2D 正交 + 屏幕中割草。** 不是 3D，也不是横版 Rogue。

| 方案 | 结论 | 原因 |
|------|------|------|
| 3D 动作场 | 否 | 模型/动画/镜头/寻路，AI 难持续出可用资源 |
| 横版 Rogue | 否 | 要关卡几何、跟镜头、单人操作；三人小队和技能表对不上 |
| **2D 屏幕中割草** | **定案** | 已有自动寻敌 + 波次 + 技能表；小队居中、敌人圆环刷入；占位色块可逐步换 Sprite |

实现锚点：`BattleField`（镜头/阵位/刷怪环）→ `BrothersBattleController` → `BattleUnit`（XY 平面，Z=0）。后期只换皮（Sprite / vfx_key），不改 System。
